...
// @version      1.4.0
// @description  树状分支 + 预览 + 一键跳转；支持最小化/隐藏与悬浮按钮恢复；快捷键 Alt+T / Alt+M；/ 聚焦搜索、Esc 关闭；拖拽移动面板；渐进式渲染；防抖监听；定位失败时提供文本预览。
// @author       Jiaoling
...
(() => {
  ...
  function ensurePanel(){
    ...
    panel.innerHTML = `
      ...
      <div id="gtt-body">
        ...
        <div id="gtt-pref">
          <span style="opacity:.65">当前版本聚焦于分支浏览与定位</span>
  ...
  async function fetchMapping(){
    ...
    for (const u of getUrls){
      try{
        const r = await origFetch(u, { credentials:'include' });
