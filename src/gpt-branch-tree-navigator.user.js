...
(() => {
  ...
  async function trySwitchBranch(nodeId){
    ...
    const runner = (async ()=>{
      ...
      try{
        const body = JSON.stringify({ current_node: targetId, conversation_id: cid });
        const res = await origFetch(url, { method: 'PATCH', credentials: 'include', headers: withAuthHeaders({'content-type':'application/json'}), body });
        ...
        if (!mapping && prefs.forceHardReload){ location.reload(); return true; }
