...
(() => {
  ...
  const CONFIG = {
    ...
    BRANCH_FETCH_RETRIES: 4,
    BRANCH_RETRY_GAP_MS: 480,
  ...
  const snapshotCache = { cid: null, payload: null };
  let mappingFetchPromise = null;
  let branchSwitchPromise = null;

  function ingestMapping(raw, opts={}){
    try{
      if (!raw) return false;
      const mapping = raw?.mapping && typeof raw.mapping === 'object' ? raw.mapping : (typeof raw === 'object' && !Array.isArray(raw) ? raw : null);
      if (!mapping || Array.isArray(mapping)) return false;
      LAST_RAW_JSON = raw?.mapping ? raw : { mapping };
      LAST_MAPPING = mapping;
      if (opts.render && LAST_MAPPING){ buildTreeFromMapping(LAST_MAPPING); }
      return true;
    }catch(err){ log('ingest mapping fail', err); return false; }
  }

  function clearSnapshotCache(){ snapshotCache.cid = null; snapshotCache.payload = null; }

  function currentNodeMatches(targetId){
    if (!targetId) return false;
    const meta = LAST_RAW_JSON || {};
    const candidates = [
      meta.current_node,
      meta.currentNode,
      meta.current_leaf,
      meta.currentLeaf,
      meta.current_leaf_message_id,
      meta.currentLeafMessageId,
      meta.current_leaf_message,
      meta.currentLeafMessage
    ];
    return candidates.filter(Boolean).some(id => String(id) === String(targetId));
  }

  function buildPatchPayload(targetId, cid){
    const payload = { conversation_id: cid, current_node: targetId };
    const meta = LAST_RAW_JSON || {};
    const ensureKeys = (keys)=>{ keys.forEach(key=>{ if (meta && key in meta){ payload[key] = targetId; } }); };
    ensureKeys(['current_leaf_message_id','current_leaf','currentLeaf','currentLeafMessageId','currentLeafMessage']);
    if (!('current_leaf_message_id' in payload)) payload.current_leaf_message_id = targetId;
    if ('override_current_leaf' in meta) payload.override_current_leaf = true;
    if ('overrideCurrentLeaf' in meta) payload.overrideCurrentLeaf = true;
    return payload;
  }

  async function fetchMappingAfterSwitch(targetId){
    for (let i=0;i<CONFIG.BRANCH_FETCH_RETRIES;i++){
      if (i === 0){ await sleep(CONFIG.REFRESH_DELAY_MS); }
      else { await sleep(CONFIG.BRANCH_RETRY_GAP_MS); }
      clearSnapshotCache();
      const mapping = await fetchMapping({ forceNetwork:true, skipSnapshot:true });
      if (mapping && currentNodeMatches(targetId)) return mapping;
    }
    return LAST_MAPPING;
  }

  function findConversationPayload(obj, depth=0){
    if (!obj || typeof obj !== 'object' || depth > 8) return null;
    if (!Array.isArray(obj) && obj.mapping && typeof obj.mapping === 'object' && !Array.isArray(obj.mapping)){
      if ('current_node' in obj || 'conversation_id' in obj) return obj;
    }
    const entries = Array.isArray(obj) ? obj : Object.entries(obj).filter(([k]) => k !== 'mapping');
    for (const entry of entries){
      const value = Array.isArray(obj) ? entry : entry[1];
      const found = findConversationPayload(value, depth + 1);
      if (found) return found;
    }
    return null;
  }

  function extractMappingFromSnapshot(){
    try{
      const cid = getConversationId();
      if (!cid) return null;
      if (snapshotCache.cid === cid && snapshotCache.payload?.mapping){ return snapshotCache.payload; }
      const script = document.querySelector('script#__NEXT_DATA__');
      if (!script?.textContent) return null;
      const data = JSON.parse(script.textContent);
      const payload = findConversationPayload(data);
      if (payload?.mapping && typeof payload.mapping === 'object'){
        snapshotCache.cid = cid;
        snapshotCache.payload = payload;
        return payload;
      }
    }catch(err){ log('snapshot parse error', err); }
    return null;
  }
    ...
    window.fetch = async (...args)=>{
      ...
      try{
        ...
        if (/\/backend-api\/conversation\//.test(url)) {
          ...
          if (json?.mapping){
            ingestMapping(json, {render:true});
            const cid = getConversationId();
            if (cid){ snapshotCache.cid = cid; snapshotCache.payload = json; }
          }
        }
      }catch(_) {}
      return res;
  ...
  async function fetchMapping(opts={}){
    const { forceNetwork=false, skipSnapshot=false } = opts;
    const cid = getConversationId();
    if (!cid) return null;

    if (!forceNetwork && !skipSnapshot){
      const payload = extractMappingFromSnapshot();
      if (payload?.mapping){
        ingestMapping(payload);
        return LAST_MAPPING;
      }
    }

    if (!forceNetwork && mappingFetchPromise){
      try{ return await mappingFetchPromise; }
      catch(_){ /* ignore, fallback to new fetch */ }
    }
    ...
    const runner = (async ()=>{
      ...
      for (const u of getUrls){
        try{
          ...
          if (myTok !== fetchCtl.token) return LAST_MAPPING;
          if (r.ok){
            const j = await r.json().catch(()=>null);
            if (j?.mapping){
              if (myTok === fetchCtl.token){
                ingestMapping(j);
                snapshotCache.cid = cid; snapshotCache.payload = j;
              }
              return j.mapping;
            }
    ...
    if (!forceNetwork) mappingFetchPromise = runner;
    const out = await runner;
    if (mappingFetchPromise === runner) mappingFetchPromise = null;
    return out;
  }
  ...
  async function trySwitchBranch(nodeId){
    ...
    if (branchSwitchPromise){
      try{ await branchSwitchPromise; }
      catch(_){ /* ignore */ }
    }
    const runner = (async ()=>{
      const cid = getConversationId();
      ...
      try{
        const payload = buildPatchPayload(targetId, cid);
        const res = await origFetch(url, { method: 'PATCH', credentials: 'include', headers: withAuthHeaders({'content-type':'application/json'}), body: JSON.stringify(payload) });
        log('PATCH current_node ->', res.status, 'target=', targetId);
        if (!res.ok) return false;
        const mapping = await fetchMappingAfterSwitch(targetId);
        const success = currentNodeMatches(targetId);
        if (!success && prefs.forceHardReload){ location.reload(); return true; }
        const linear = harvestLinearNodes();
        if (mapping){ buildTreeFromMapping(mapping); }
        else if (LAST_MAPPING){ buildTreeFromMapping(LAST_MAPPING); }
        else { buildTreeFromLinear(linear); }
        return success;
      }catch(e){ log('PATCH error', e); return false; }
    })();
    branchSwitchPromise = runner;
    try{ return await runner; }
    finally{ if (branchSwitchPromise === runner) branchSwitchPromise = null; }
  }
  ...
  async function rebuildTree(opts={}){
    ensureFab(); ensurePanel();
    if (opts.hard){ LAST_MAPPING=null; LAST_LINEAR=[]; LAST_RAW_JSON=null; clearSnapshotCache(); }
    harvestLinearNodes(); // 先收集一次，确保 DOM_BY_SIG/ID 准备就绪
    if (opts.forceFetch || !LAST_MAPPING){ await fetchMapping({ forceNetwork: !!opts.forceFetch }); }
