...
(() => {
  ...
  injectStyle(`
    ...
    #gtt-tree{flex:1;min-height:0;overflow:auto;padding:8px 6px 10px}
    ...
    .gtt-node .head{display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-bottom:4px}
    .gtt-node .role{font-size:11px;font-weight:600;opacity:.85}
    .gtt-node .meta{opacity:.65;font-size:11px;margin-left:auto}
    .gtt-node .pv{display:-webkit-box;-webkit-box-orient:vertical;-webkit-line-clamp:3;opacity:.88;margin-left:0;white-space:normal;overflow:hidden;text-overflow:ellipsis;line-height:1.5}
  ...
  function renderTreeGradually(targetEl, treeData){
    ...
    const createItem = (node)=>{
      ...
      const role = document.createElement('span'); role.className='role'; role.textContent = node.role==='user' ? '用户' : '助手';
      head.append(badge, role);
      ...
      const pv = document.createElement('div'); pv.className='pv'; pv.textContent = preview(node.text);

      item.append(head, pv); item.addEventListener('click', ()=>jumpTo(node));
      return item;
    };
